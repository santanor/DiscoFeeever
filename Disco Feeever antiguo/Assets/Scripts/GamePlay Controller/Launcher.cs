...
public class Launcher : MonoBehaviour {
	...
	void Start () {
		...
		minutesPerMoscon = 60 / moscones.Count;
		FindObjectOfType<HourGUITextController>().SetHour(jObject["levels"]["level"][Level]["implement"]["-hour"].str);
	}
	...
	void Update () {
		...
		if(moscones.Count > 0)
		{
			if(moscones[0].GetTimer() < timer)
			{
				...
				moscones[0].Launch();
				FindObjectOfType<HourGUITextController>().IncrementateMinutes(minutesPerMoscon);
				moscones.RemoveAt(0);
			}
		}
	}
